...
export function AdministrativeApproval() {
  ...
  // Fetch New Proposals (revised = false) for Admin Approval (Committee ID = 4)
  const { data: pendingProposals = [], isLoading: isLoadingPending } = useQuery(
    {
      queryKey: ["adminApprovProposals", "new"],
      queryFn: async () => {
        const response = await axiosPrivate.get(
          "/api/v1/Committees/4/queue?revised=false"
        );
  ...
  // Fetch Revised Proposals (revised = true) for Admin Approval
  const { data: revisionList = [], isLoading: isLoadingRevised } = useQuery({
    queryKey: ["adminApprovProposals", "revised"],
    queryFn: async () => {
      const response = await axiosPrivate.get(
        "/api/v1/Committees/4/queue?revised=true"
      );
      return response.data;
    },
  });
  ...
  const evaluateMutation = useMutation({
    mutationFn: async (actionDecision: number) => {
      ...
      // MOM file is required only for Approval. Admin Approval MoM DocType is 6
      if (actionDecision === DecisionEnum.Approve && momFile) {
        uploadPromises.push(createUploadConfig(momFile, 6));
      }
      ...
      const formattedMomDate = momDate
      ...
      const payload = {
        ...
        committee: CommitteeType.AdministrativeApproval,
        meetingDate: meetingDateTime,
        meetingTime: meetingTime,
        decision: actionDecision,
        momDate: formattedMomDate,
    ...
    onSuccess: (actionDecision) => {
      queryClient.invalidateQueries({ queryKey: ["adminApprovProposals"] });

      const propRef = selectedProposal.proposalRefNo || selectedProposal.id;

      if (actionDecision === DecisionEnum.Approve) {
        toast.success(`${propRef} forwarded successfully to SDMA`);
  ...
  const revisedEvaluateMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload = {
        ...
        meetingDate: selectedProposal.lastMeetingDate
          ? new Date(selectedProposal.lastMeetingDate).toISOString()
          : new Date().toISOString(),
      ...
      if (revisedMomFile) {
        ...
        formData.append("ownerId", proposalId);
        formData.append("documentType", "6"); // Admin Approval MoM DocType is 6
  ...
  const handleForwardRevisedToSDMA = () => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Administrative Approval review and evaluation
        </p>
      </div>
      ...
      <div className="flex gap-4">
        <button
          ...
          className={`px-5 py-2 rounded-lg font-medium ${
            activeTab === "new" ? "bg-primary text-white" : "bg-muted hover:bg-muted/80"
          }`}
        >
          New Proposals
        </button>
        <button
          ...
          className={`px-5 py-2 rounded-lg font-medium ${
            activeTab === "revised" ? "bg-primary text-white" : "bg-muted hover:bg-muted/80"
          }`}
        >
          Revised Proposals
        </button>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    type="date"
                    value={selectedProposal.lastMeetingDate?.substring(0, 10) || ""}
                    disabled
          ...
          {activeTab === "revised" ? (
            <div className="bg-card border border-border rounded-xl p-6 shadow-sm mt-6">
              <h3 className="mb-4 text-lg font-bold border-b pb-2">
                Revised AA Evaluation : {selectedProposal.proposalRefNo || selectedProposal.id}
              </h3>
              <div className="space-y-6">
                ...
                <div className="flex justify-between mt-6 pt-6 border-t">
                  ...
                  <button
                    onClick={handleForwardRevisedToSDMA}
                    disabled={revisedEvaluateMutation.isPending}
                    className="px-6 py-3 bg-blue-600 text-white rounded-lg flex items-center gap-2 hover:bg-blue-700 transition-colors font-medium text-sm disabled:opacity-50 shadow-sm"
                  >
            ...
            <div className="bg-card border border-border rounded-xl p-6 shadow-sm mt-6">
              <h3 className="mb-4 text-lg font-bold border-b pb-2">
                Administrative Approval Evaluation : {selectedProposal.proposalRefNo || selectedProposal.id}
              </h3>
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                ...
                {decision === "approve" && (
                  <div className="animate-in fade-in slide-in-from-top-4 duration-300 p-4 bg-green-50/50 rounded-xl border border-green-100 space-y-4">
                    <div>
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2 text-green-800">
                        Upload AA Minutes of Meeting (MOM)
                      </label>
                      <input
                        type="file"
                        accept=".pdf,.doc,.docx"
