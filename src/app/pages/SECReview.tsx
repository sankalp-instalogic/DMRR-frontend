...
export function SECReview() {
  ...
  // Fetch New Proposals (revised = false) for SEC (Committee ID = 3)
  const { data: pendingProposals = [], isLoading: isLoadingPending } = useQuery(
    {
      queryKey: ["secProposals", "new"],
      queryFn: async () => {
        const response = await axiosPrivate.get(
          "/api/v1/Committees/3/queue?revised=false"
        );
  ...
  // Fetch Revised Proposals (revised = true) for SEC
  const { data: revisionList = [], isLoading: isLoadingRevised } = useQuery({
    queryKey: ["secProposals", "revised"],
    queryFn: async () => {
      const response = await axiosPrivate.get(
        "/api/v1/Committees/3/queue?revised=true"
      );
      return response.data;
    },
  });
  ...
  const { data: timelineData = [], isLoading: isLoadingTimeline } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await axiosPrivate.get(
        `/api/v1/Proposals/${proposalId}/timeline`
      );
      return response.data;
    },
  ...
  // Extract SEC Revision details from timeline data
  const secRevisionDetails = timelineData.find(
    (entry: any) => entry.status === "SEC Revision"
  );

  // Format the date to YYYY-MM-DD for the input[type="date"]
  const lastMeetingDate = secRevisionDetails?.meetingDate
    ? secRevisionDetails.meetingDate.substring(0, 10)
    : "";

  const reasonForRevision =
    secRevisionDetails?.revisionReson || 
    secRevisionDetails?.revisionReason || 
  ...
  const evaluateMutation = useMutation({
    mutationFn: async (actionDecision: number) => {
      ...
      // MoM file for SEC is DocType 5
      if (actionDecision === DecisionEnum.Approve && momFile) {
        uploadPromises.push(createUploadConfig(momFile, 5));
      }
    ...
    onSuccess: (actionDecision) => {
      queryClient.invalidateQueries({ queryKey: ["secProposals"] });

      const propRef = selectedProposal.proposalRefNo || selectedProposal.id;

      if (actionDecision === DecisionEnum.Approve) {
        toast.success(`${propRef} forwarded successfully to Administrative Approval`);
        navigate("/administrative-approval");
  ...
  const revisedEvaluateMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload = {
        ...
        momDate: revisedApprovalDate
          ...
          : new Date().toISOString(),
        isRevisedRound: true,
        members: [], // Empty members array as per pattern
      };

      await axiosPrivate.post("/api/v1/Committees/evaluation", payload);

      // 2. Only if evaluation succeeds, upload MOM Document for SEC (DocType 5)
      if (revisedMomFile) {
        const formData = new FormData();
        formData.append("ownerType", "1"); // Proposal
        formData.append("ownerId", proposalId);
        formData.append("documentType", "5"); // SEC MoM is 5
    ...
    onSuccess: () => {
      ...
      const propRef = selectedProposal.proposalRefNo || selectedProposal.id;
      toast.success(
        `${propRef} forwarded successfully to Administrative Approval`
      );
      navigate("/administrative-approval");
      resetForm();
    },
  ...
  const handleForwardRevisedToNextStage = () => {
            ...
            <div className="bg-card border border-border rounded-xl p-6 shadow-sm mt-6">
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-between items-center mt-6 pt-6 border-t">
                  ...
                  <button
                    onClick={handleForwardRevisedToNextStage}
                    disabled={revisedEvaluateMutation.isPending}
                  ...
                  >
                    ...
                    Forward to Administrative Approval
                  </button>
                </div>
              </div>
            </div>
